...
- **Tap to inspect** — tap any aircraft for a detail panel, then tap the panel itself to step through three pages: summary (callsign, ICAO24 address, altitude, speed, heading, type code, **GND** flag), position (lat/lon, squawk code, distance from home, position age), and identity (registration, full aircraft description, operator/airline). See [Using the Device](using-the-device.md#touch-select-an-aircraft).
...
- **Emergency squawk detection** — 7500 (hijack), 7600 (radio failure), and 7700 (general emergency) are highlighted in red, auto-follow onto the aircraft, and trigger a bezel ring-flash and an optional buzzer. See [Emergency Alerts](emergency-alerts.md).
...
- **Firmware updates** — check for and install new firmware from the same always-on web config page, with changenotes shown before you confirm; always a manual, explicit action, never automatic.
