...
- **Battery sensing is not configured for M5Dial in M5Unified.** Boards that support it (e.g. the near-identical M5DinMeter) name an ADC channel and divider ratio in `Power_Class::begin()`; `board_M5Dial` names none, so `getBatteryLevel()` falls through to `return -2`. The gauge treats an implausible reading as "no battery" and draws nothing, so this fails quietly by design — use the `BATT` command to see the raw values before assuming the gauge itself is broken.
