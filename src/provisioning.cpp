...
// ── Stored credentials ────────────────────────────────────────────────────────
static char _osUser[64] = "";
static char _osPass[64] = "";

const char* openskyUser() { return _osUser; }
const char* openskyPass() { return _osPass; }
...
void runProvisioning() {
    ...
    Preferences prefs;
    prefs.begin("flightdial", /*readOnly=*/false);
    prefs.getString("os_user", _osUser, sizeof(_osUser));
    prefs.getString("os_pass", _osPass, sizeof(_osPass));
    ...
    // Custom portal fields (OpenSky creds optional, location pre-filled)
    WiFiManagerParameter osUserParam(
        "os_user", "OpenSky username (optional)", _osUser, 63);
    WiFiManagerParameter osPassParam(
        "os_pass", "OpenSky password (optional)", "", 63);
    ...
    wm.setSaveParamsCallback([&]() {
        ...
        prefs.putString("os_user", _osUser);
        prefs.putString("os_pass", _osPass);
    ...
    Serial.printf("[Provision] Connected: %s  OpenSky user: %s  Home: %.6f, %.6f\n",
